...
const storage = multer.diskStorage({
  ...
  fileFilter: (
    req: {},
    res: { redirect: (xx: String) => {} },
    file: { mimetype: String; originalname: String },
    cb: (arg0: null, dx: boolean) => {}
  ) => {
    if (
      fs.existsSync(
        path.join(__dirname, '../../assets/images/full/' + file.originalname)
      )
    ) {
      res.redirect(
        `/api/resize?file=${file.originalname}&width=350&height=350`
      );
    } else if (
      ...
      cb(null, true);
    } else console.log('Not expected file format');
...
upload.post(
  ...
  function (req: any, res: any, next) {
    res.redirect(
      `/api/resize?file=${req.file.originalname}&width=${req.body.width}&height=${req.body.height}`
    );
